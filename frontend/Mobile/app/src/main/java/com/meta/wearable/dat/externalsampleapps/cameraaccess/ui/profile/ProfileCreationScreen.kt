...
import com.meta.wearable.dat.externalsampleapps.cameraaccess.ui.theme.VisorPalette
...
fun ProfileCreationScreen(
    palette: VisorPalette = VisorPalette.Current,
    speak: (String) -> Unit = {},
    ...
    Column(
        ...
        when (step) {
            Step.NAME -> BigTextField(palette, profile.displayName, "Your name") {
            ...
            Step.DESCRIBE -> BigTextField(palette, profile.visionDescription, "In your own words…") {
            ...
            Step.VISION -> VisionType.entries.forEach { type ->
                ...
                BigChoiceButton(type.label(), selected, palette) {
        ...
        if (step != Step.DONE) {
            TextButton(onClick = ::next, modifier = Modifier.fillMaxWidth()) {
                Text("Skip for now", color = palette.textSecondary, fontSize = 20.sp)
            }
        }
    }
}
...
private fun BigTextField(
    ...
    OutlinedTextField(
        ...
        modifier = Modifier.fillMaxWidth(),
        textStyle = TextStyle(fontSize = 26.sp, color = palette.textPrimary),
        placeholder = { Text(placeholder, fontSize = 26.sp, color = palette.textSecondary) },
...
private fun BigChoiceButton(
    ...
    val fg = if (selected) palette.onAccent else palette.textPrimary
    OutlinedButton(
        onClick = onClick,
        modifier = Modifier
            .fillMaxWidth()
...
private fun ProfileCreationPreviewDarkReference() {
    ProfileCreationScreen(palette = VisorPalette.Cerebro)
}
